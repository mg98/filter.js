...
export function extractWords(constraints: string): Word[] {
    ...
    for (let i = 0; i < s.length; i++) {
        ...
        for (let depth = 0; depth < groupDepth; depth++) {
            if (words[words.length-1].type !== WordType.Group || !Array.isArray(words[words.length-1].words)) throw new Error('internal error');
        ...
        if (s[i] === '(') {
            if (lastWord.type === WordType.Operator && lastWord.value && ['in', 'not in'].includes(lastWord.value.toString())) {
                arrayRec = true;
                words.push({
                    type: WordType.Value,
                    value: []
